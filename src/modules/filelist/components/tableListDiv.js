...
export const TableListDiv = ({ props, columns, data, Actions, fn, startFileDownloadFunc, setHeight, getIndex, filterBoxNone, rightBox, centerBox, callHeightFunc, paddingShort, getWidth, actionClass }) => {
    ...
    function DefaultColumnFilter({
        ...
        return (
            <Input
                ...
                style={{ width: '95%', height: "30px", fontSize: "12px" }}
            />
        )
    }
    ...
    const RenderRow = React.useCallback(
        ({ index, style }) => {
            ...
            return (
                ...
                >
                    {row.cells.map(cell => {
                        {/* console.log(cell.row.original,'===========cell') */ }
                        i++;
                        return (
                            <>
                                <div {...cell.getCellProps()} className={'td ' + cell.render('classNameGet')}>
                                    {(cell.column.id === "org_name") ? <span className='fileTypeName' onClick={() => startFileDownloadFunc(cell.row.original)}>{cell.row.original.org_name}</span> : cell.render('Cell')}
                                    {/* {cell.render('Cell')} */}
                                </div>
                                {i === getIndex ?
